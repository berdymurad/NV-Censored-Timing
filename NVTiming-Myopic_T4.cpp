//============================================================================
// Name        : NVTiming-T4-Myopic.cpp
// Author      : Tong
// Version     :
// Copyright   : ...
// Description : NVTiming - myopic policy: use optimal Bayesian learning, but do not optimize inventory, use myopic level instead
//						  - heuristic policy: use heuristic Bayesian learning (omit actual timing, extrapolate demand), but optimize inventory level
//============================================================================
...
double G_COT_M(int n, int x, double alpha, double beta);

double V_COT_M(int n, double alpha, double beta)
{
	int x_opt = find_x_myopic(alpha, beta);

	double v_max = G_COT_M(n, x_opt, alpha, beta);
...
double G_COT_M(int n, int x, double alpha, double beta)
{
	...
	if (n<N)
	{
		...
		else
		{
			...
			for (int d=0; d<=x-1; d++)
				out += V_COT_M(n+1, alpha+d, beta+1) * NegBinomial(d, r, p);
			...
			for (int i=1;i<=T_STEP;i++)  //t=i*delta_t //for (double t=0; t<1; t+=delta_t)
				out += V_COT_M(n+1, alpha+x, beta+(i-0.5)*delta_t) * InvBeta2((i-0.5)*delta_t, x, alpha, beta) * delta_t;
...
double G_COT_H(int n, int x, double alpha, double beta);

double V_COT_H(int n, double alpha, double beta)
{
	int x = 0;
	int x_low = find_x_myopic(alpha, beta);
	int x_opt = x_low;
	double v_max = G_COT_H(n, x_low, alpha, beta);

	for (x=x_low+1;;x++)
	{
		double temp = G_COT_H(n, x, alpha, beta);
...
double G_COT_H(int n, int x, double alpha, double beta)
{
	...
	if (n<N)
	{
		...
		else
		{
			...
			//#pragma omp parallel for schedule(dynamic) reduction(+:out)
			for (int d=0; d<=x-1; d++)
				out += V_COT_H(n+1, alpha+d, beta+1) * NegBinomial(d, r, p);
			...
			#pragma omp parallel for schedule(dynamic) reduction(+:out)
			for (int i=1;i<=T_STEP;i++)  //t=i*delta_t //for (double t=0; t<1; t+=delta_t)
				out += V_COT_H(n+1, alpha + x/((i-0.5)*delta_t), beta+1 ) * InvBeta2((i-0.5)*delta_t, x, alpha, beta) * delta_t;
...
int main(void)
{
	...
	//Open output file
	if ((fp=fopen("NVTiming-Myopic-T4_50.txt","w"))==NULL)
	{
		printf("%s","Open file \"NVTiming-Myopic.txt\" error!!");
		return -1;
	}
	...
	for (cost=0.2;cost<1.95;cost+=0.1)
	{
		...
		V_COT_M(1, alpha0, beta0);
		//V_COT_H(1, alpha0, beta0);
